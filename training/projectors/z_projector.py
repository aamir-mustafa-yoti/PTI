...
def project(
    ...
    w_samples = G.mapping(torch.from_numpy(z_samples).to(device), None)  # [N, L, C] = (10000, 18, 512)
    w_samples = w_samples[:, :1, :].cpu().numpy().astype(np.float32)  # [N, 1, C] = (10000, 1, 512)
    w_avg = np.mean(w_samples, axis=0, keepdims=True)  # [1, 1, C] = (1, 1, 512)
